...
bool CFInput::getgridcell(Gridcell& gridcell) {
   ...
   bool grid_found = false; // (Wim Verbruggen)

   // If an error occurs (eg. no soil), we move on to the next gridcell.
   while(!grid_found){
	...
	dprintf("\nCommencing simulation for gridcell at (%g,%g). ", lon, lat);
	if (current_gridcell->descrip != "") {
		dprintf("Description: %s\n", current_gridcell->descrip.c_str());
	}
	dprintf("Using soil data and Nitrogen deposition for (%3.1f,%3.1f)\n", cru_lon, cru_lat);
	
	// Try finding the soil data as well. If not found, move to next gridcell.
	grid_found = true;
	try{
		...
		if(soilEx==5017)
		dprintf(">>> No soil found for these coordinates. Moving on to the next gridcell.\n");
		grid_found = false;
		++current_gridcell;
	}
	...
	* we need to use the RCP data for the last decade. */
	// Also, if the coordinates are not found, we move on to the next gridcell without crashing the model. (Wim Verbruggen)
	if(grid_found){
		try{
		  ...
		  if(exNDEP==332211){
			dprintf(">>> No NDEP records found for these coordinates. Moving on to the next gridcell.\n");
			grid_found = false;
			++current_gridcell;
		  } else {
			fail("Something went wrong when reading out NDEP. I'm calling out from cfinput.cpp here.");
		  }	
		}
	}
...
bool CFInput::getclimate(Gridcell& gridcell) {
	...
	//dprintf(">>> DEBUG: year=%i CO2=%g \n",date.get_calendar_year(),climate.co2);
	...
	// Additional sanity check (Wim Verbruggen, 2021)
	if(climate.temp > 1000){
		dprintf(">>> MAJOR FAIL: air surface temperature higher than 1000 degC in driver data. Moving on to next gridcell.\n");
		++current_gridcell;
		return false;
	}
