...
coord coordFix(coord C){
	double lon = round(C.first*100)/100;
	double lat = round(C.second*100)/100;
	coord Cfixed(lon,lat);
	return Cfixed;
}
...
coord SoilInput::find_closest_point(double searchradius, coord C) {
	...
	// Apparently the "count" function has some issues when comparing floats
	// eg. coordinates such as (-16.1,12.4) can not be found, even though they are
	// present in the soil DB. The solution is to round them again, with the function below.
	// (Wim Verbruggen, 2021)
	C = coordFix(C);
