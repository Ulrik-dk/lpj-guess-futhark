...
void Soil::update_soil_diffusivities(const int& daynum, bool ansoln) {
	...
	for (int i = IDX; i<NLAYERS; i++) {
		...
		if (Frac_air[i] < -0.00001)
			fail("Illegal Frac_air value in Soil::update_soil_diffusivities: %g. \n Layer=%i por[i]=%g Frac_water[i]=%g Frac_water_belowpwp[i]=%g wcont=%g soiltype.awc=%g", Frac_air[i],i,por[i],Frac_water[i],Frac_water_belowpwp[i],wcont[i],soiltype.awc[i]);
